...
public class CartController {
    ...
    // 添加商品
    @PostMapping("/add")
    public ResponseEntity<Map<String, Object>> addToCart(@RequestBody Map<String, Integer> body, HttpServletRequest request) {
        Map<String, Object> res = new HashMap<>();
        User user = (User) request.getSession().getAttribute("currentUser");
        if (user == null) return ResponseEntity.status(401).body(Map.of("message", "请先登录"));

        int ingredientId = body.get("ingredientId");
    ...
    // 移除商品
    @PostMapping("/remove")
    public ResponseEntity<String> remove(@RequestBody Map<String, Integer> body) {
