...
-- 食谱表 (已修正：补充 user_id, status, is_recommended)
CREATE TABLE IF NOT EXISTS recipes (
    ...
    ingredients TEXT,
    steps TEXT,
    is_recommended BOOLEAN DEFAULT FALSE, -- 新增：是否推荐
    status INT DEFAULT 1, -- 新增：状态 (1=正常/已审核)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- 烹饪记录表 (已修正：去重并统一使用 image 字段)
CREATE TABLE IF NOT EXISTS cooking_records (
                                               id INT AUTO_INCREMENT PRIMARY KEY,
                                               user_id INT NOT NULL,
                                               recipe_id INT NOT NULL,
                                               note TEXT, -- 心得
                                               image VARCHAR(200), -- 成品图 (Java代码中使用的是 image)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id),
    FOREIGN KEY (recipe_id) REFERENCES recipes(id)
    );
...
-- 插入默认菜系
INSERT IGNORE INTO cuisines (name, code, description) VALUES
('中餐', 'chinese', '中国传统美食'),
('西餐', 'western', '欧美风味料理'),
('日料', 'japanese', '日本传统料理'),
('韩式', 'korean', '韩国特色美食'),
...
-- 插入默认食材分类
INSERT IGNORE INTO ingredient_categories (name, code) VALUES
('新鲜蔬菜', 'vegetables'),
('肉类禽蛋', 'meat'),
('海鲜水产', 'seafood'),
('奶制品', 'dairy'),
...
-- 初始化食材数据 (保留你原有的数据)
INSERT IGNORE INTO ingredients (id, name, category_id, price, unit, stock, image) VALUES
